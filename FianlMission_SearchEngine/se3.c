...
char words_dictionary[4700000], words_stopwords[10000];
int dictionary_len, stopwords_len;
char article[35000000];                                        //储存网页集合
...
int main(int argc, char *argv[]) {
    ...
    FILE *fp_results = fopen("results.txt", "w");
//读入字典单词生成字典树
    dictionary_len = fread(words_dictionary, sizeof(char), 4700000, fp_dictionary);
    for (i = 0, p = 0; i < dictionary_len; i++) {
        if (words_dictionary[i] == '\r' || words_dictionary[i] == '\n') {
            words[p][0] = 1;
            p = 0;
            continue;
        } else {
            temp = words_dictionary[i] - 'a';
            if (!trie[p][temp]) {
                trie[p][temp] = pos++;
            }
            p = trie[p][temp];
...
/*    while (~fscanf(fp_dictionary, "%s", word_dictionary)) {     
        ...
        words[p][0] = 1;
    }*/ /*优化2：用fread统一读是否比fscanf分行读更快*/ //(快了0.1s)
//    fclose(fp_dictionary);
//读入停用词从单词表中删去
    stopwords_len = fread(words_stopwords, sizeof(char), 10000, fp_stopwords);
    for (i = 0, p = 0; i < stopwords_len; i++) {
        if (words_stopwords[i] == '\r' || words_stopwords[i] == '\n') {
            words[p][0] = 0;
            p = 0;
            continue;
        } else {
            p = trie[p][words_stopwords[i] - 'a'];
            if (!p) {                                           //这里一定要判 有停用词不在字典里
                while (words_stopwords[i] > 'a' && words_stopwords[i] < 'z') {
                    i++;
                }
            }
        }
    }
/*    while (~fscanf(fp_stopwords, "%s", word_stopwords)) {       
...
//处理网页集合
    article_len =  fread(article, sizeof(char), 35000000, fp_article);
//    fclose(fp_article);
